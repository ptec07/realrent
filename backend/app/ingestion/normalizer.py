...
def normalize_rental_transaction(row: dict[str, str], *, source_type: HousingType | str) -> dict[str, Any]:
    ...
    monthly_rent = _parse_int(_pick(row, "월세금액", "월세"))
    contract_year_month = _parse_year_month(_pick(row, "계약년월"))

    normalized = {
        "source_type": normalized_source_type,
        "rent_type": RentType.jeonse if monthly_rent == 0 else RentType.monthly,
        "region_sido": _pick(row, "시도", "지역시도"),
        "region_sigungu": _pick(row, "시군구", "지역시군구"),
        "region_dong": _pick_optional(row, "법정동", "동"),
        "region_code_5": _pick(row, "지역코드", "법정동시군구코드", "LAWD_CD"),
        "building_name": _pick(row, "아파트", "단지명", "오피스텔", "건물명"),
        "address_jibun": _pick_optional(row, "지번"),
        "area_m2": Decimal(_pick(row, "전용면적", "전용면적(㎡)").replace(",", "")),
        "floor": _parse_optional_int(_pick_optional(row, "층")),
        "built_year": _parse_optional_int(_pick_optional(row, "건축년도", "건축연도")),
        "contract_date": _parse_contract_date(_pick(row, "계약년월"), _pick(row, "계약일")),
        "contract_year_month": contract_year_month,
        "deposit_amount_manwon": _parse_int(_pick(row, "보증금액", "보증금")),
